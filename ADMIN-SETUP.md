...
后台使用 Cloudflare Pages Functions + KV + R2，不需要单独服务器。
...
```text
yunlian-curtain-media
```

4. 打开 Pages 项目 `yunlian-curtain`。
5. 进入 `设置` -> `函数` -> `R2 存储桶绑定`。
6. 添加绑定：

```text
变量名称：SITE_MEDIA
R2 存储桶：yunlian-curtain-media
```
...
- SEO 标题、搜索摘要、分享图片
- 首屏标题、说明、图片
- 服务项、案例、案例图片上传、案例视频上传、价格套餐、咨询流程
- 底部转化文案和页脚说明
...
- 上传文件建议控制在 100MB 以内；手机视频过大时先压缩再上传。
